...
int profundidad(nodo n) const; //profundidad de un nodo (ej4 p1)
int altura(nodo n) const; // altura de un nodo (ej4 p1)
...
//ejercicio 4a p1 - añadir operacion que calcule la profundidad de un nodo
template <typename T>
inline int Abin<T>::profundidad(nodo n) const{
    assert(n >= 0 && n < numNodos); // Nodo válido
        int prof=0;
        while (nodos[n].padre != NODO_NULO) //si tiene nodo padre
        {
            prof++;//se incrementa prof en 1 
            n=nodos[n].padre;// asigno a n la posicion del padre
        }
    return prof;    
}

//ejercicio 4b p1 - añadir operacion que calcule la altura de un nodo
template <typename T>
inline int Abin<T>::altura(nodo n) const{
    assert(n >= 0 && n < numNodos); // Nodo válido
    int altd = 0, alti = 0;
    if (nodos[n].hder != NODO_NULO) {
        altd = 1 + altura(nodos[n].hder);
    }
    if (nodos[n].hizq != NODO_NULO) {
        alti = 1 + altura(nodos[n].hizq);
    }
