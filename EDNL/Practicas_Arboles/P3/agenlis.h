...
void Agen<T>::eliminarHijoIzqdo(nodo n){
    ...
    assert(nodos[hizqdo].hijos.primera() == nodos[hizqdo].hijos.fin()); // Lista vacía, hijo izq. de n es una hoja.
...
void Agen<T>::eliminarHermDrcho(nodo n){
    ...
    assert(nodos[hdrcho].hijos.primera() == nodos[hdrcho].hijos.fin());// Lista vacía,  // drcho. de n es hoja.
...
inline typename Agen<T>::nodo Agen<T>::hermDrcho(nodo n) const{
    ...
    if (n == 0) // n es la raíz.
        ...
        while (n != Lhp.elemento(p)) p = Lhp.siguiente(p);
        p = Lhp.siguiente(p);
        return p != Lhp.fin() ? Lhp.elemento(p) : NODO_NULO; // n tiene hermano drcho.
    }
}
